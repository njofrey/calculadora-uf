document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- SOLUCIÓN DEFINITIVA: UN SOLO SVG CON AMBOS DIBUJOS ---
    // Se definen ambos dibujos (paths) dentro de un único SVG.
    // El dibujo del check está oculto por defecto con style="display:none;".
    const combinedIcon = `
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path class="copy-path" stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 01-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 011.5.124m7.5 10.376h3.375c.621 0 1.125-.504 1.125-1.125V11.25c0-4.46-3.243-8.161-7.5-8.876a9.06 9.06 0 00-1.5-.124H9.375c-.621 0-1.125.504-1.125 1.125v3.5m7.5 10.375H9.375a1.125 1.125 0 01-1.125-1.125v-9.25m9.75 11.625l-3.75-3.75" />
            <path class="check-path" stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" style="display:none;" />
        </svg>
    `;
    ...
    // --- LÓGICA DE COPIADO (SIN REEMPLAZAR EL ÍCONO) ---
    resultBox.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(numberToCopy).then(() => {
            ...
            // Ocultar el dibujo de copiar y mostrar el de check
            copyIconContainer.querySelector('.copy-path').style.display = 'none';
            copyIconContainer.querySelector('.check-path').style.display = 'block';

            // Feedback de texto y flash
            ...
            setTimeout(() => {
                // Ocultar el dibujo de check y mostrar el de copiar
                copyIconContainer.querySelector('.copy-path').style.display = 'block';
                copyIconContainer.querySelector('.check-path').style.display = 'none';

                // Quitar feedback de texto y flash
    ...
    copyIconContainer.innerHTML = combinedIcon; // Cargar el SVG combinado al inicio
    getUfValue();
});
